...
public class AreaSeeHandler implements AreaCommandHandler {
    ...
    private final static String helpString = new StringJoiner(" ").add("\"see\"")
            .add("Will give you some information about your surroundings.\r\n").add("\"see [name]\"")
            .add("May tell you more about the object with that name.").toString();
    ...
    @Override
    public Reply visit(CommandContext ctx, SeeMessage sMessage) {
        if (sMessage == null) {
        ...
        if (sMessage.getThing() != null && !sMessage.getThing().isBlank()) {
            ...
            if (ctx.getCreature() != null && ctx.getCreature().isInBattle()) {
                ctx.receive(SeeEvent.getBuilder()
                        .setDeniedReason("You are in a fight right now, you are too busy to examine that!").Build());
                return ctx.handled();
            }

            for (IItem ro : ctx.getArea().getItems()) {
                if (ro.CheckNameRegex(name, 3)) {
                    ctx.receive(ro.produceMessage(
                            SeeEvent.getBuilder().setExaminable(ro).addExtraInfo("You see it in the room with you. ")));
                    return ctx.handled();
                }
            }
            ...
            if (ctx.getCreature() != null) {
                ...
                for (AItem thing : creature.getEquipmentSlots().values()) {
                    if (thing.CheckNameRegex(name, 3)) {
                        ...
                        ctx.receive(SeeEvent.getBuilder().setExaminable(thing).addExtraInfo("You have it equipped. ")
                                .Build());
                        return ctx.handled();
                    }
                }
            ...
            ctx.receive(SeeEvent.getBuilder().setDeniedReason("You couldn't find " + name + " to examine. ").Build());
            return ctx.handled();
        } else {
