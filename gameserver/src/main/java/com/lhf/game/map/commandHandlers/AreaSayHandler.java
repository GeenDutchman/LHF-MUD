...
public class AreaSayHandler implements AreaCommandHandler {
    ...
    private static final String helpString = new StringJoiner(" ").add("\"say [message]\"")
            .add("Tells everyone in your current room your message").add("\r\n").add("\"say [message] to [name]\"")
            ...
            .add("\"say 'They are taking the hobbits to Isengard' to Aragorn\"").add("\r\n").toString();
    ...
    @Override
    public Reply visit(CommandContext ctx, SayMessage sMessage) {
        if (sMessage == null) {
